...
def _assert_two_abreast_passable(obstacles):
    """Static geometric guard against param drift: two dogs abreast fit a cylinder gap iff
    center_dist >= r_eff_a + r_eff_b + D_MIN. Checks only the 5 FIELD cylinders (radius
    ~0.30); door POSTS (radius ~0.15) are wall structure -- adjacent same-side posts sit
...
def main():
    ...
    _assert_two_abreast_passable(obstacles)
    ...
    planner = AStarPlanner(resolution=0.15, robot_radius=ASTAR_ROBOT_RADIUS, obstacles=obstacles,
                           x_min=-2.0, x_max=15.0, y_min=-7.0, y_max=7.0,  # covers 1.4x-scaled door
    ...
    for cyc in range(MAX_CYCLES):
        ...
        for i in DOGS:
            node_h[i].append(va.realized_node_h(x[i][:2], obstacles, margin=ROBOT_MARGIN))
