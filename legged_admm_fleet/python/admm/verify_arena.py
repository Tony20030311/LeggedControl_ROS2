...
from admm_impl import ARENAS, FORMATIONS         # noqa: E402
...
def realized_node_h(p, obstacles, margin=ROBOT_MARGIN):
    # margin MUST match the coordinator's robot_margin, else h is checked against a
    # different bubble than the one the QP enforced
    return min(nq.h_obstacle(p, o["pos"], o["radius"] + margin) for o in obstacles)
