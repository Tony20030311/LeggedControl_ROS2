...
form_half_extent() {
  ...
  # The plan is on the wire and the agents are reading it -- they rebuild their shape from
  # it -- but this CLI read is a fresh transient_local subscription every time, and after an
  # eviction it has now failed all five attempts twice in a row (d_0805_091*, d_0805_0926*)
  # on a machine that was quiet. Two runs died at phase 6 with the body-gap measurement they
  # existed to produce already safely on disk.
  #
  # So: ask the fleet's own configuration first. The formation shapes are compiled into
  # fleet_config.cpp and the half-extent is a property of the shape, not of the run; the
  # topic can only tell us the same number for the same $FORMATION. Falling back to the read
  # keeps a non-default or reassigned plan honest.
  # Values are read off fleet_config.cpp's kFormations, which is where the coordinator gets
  # them too: V is {0.808,0} and {-0.404,+-0.7}, already mean-centred, so 0.808. COLn is
  # generated by the rule col[i] = (n-1)*0.75 - i*1.50, giving a half-extent of (n-1)*0.75 --
  # 0.750 for the two-dog column an eviction leaves behind. Anything else falls through to
  # the topic read.
  case "${FORMATION:-V}" in
    V)     echo 0.808; return 0 ;;
    COL*)  local n=${FORMATION#COL}
           case "$n" in ''|*[!0-9]*) ;; *) python3 -c "print('%.3f' % ((int($n)-1)*0.75))"; return 0 ;; esac ;;
  esac
