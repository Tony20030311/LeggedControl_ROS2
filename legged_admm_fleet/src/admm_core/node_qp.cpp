...
std::vector<double> NodeSubproblem::q_(const Eigen::MatrixXd& x_des,
    ...
    // 單狗（無鄰居/無編隊）時上游會給空向量/空矩陣：consensus 與 formation 項數學上為 0，跳過。
    // 三狗時 size 恆為滿，數值行為與 oracle bit-identical 不變。
    if (consensus_target != nullptr && consensus_target->size() == n_xi_ && rho_consensus_ > 0.0)
        for (int j = 0; j < n_xi_; ++j) q[j] += -rho_consensus_ * (*consensus_target)(j);
    if (formation_grad != nullptr && formation_grad->rows() >= N && w_form_ > 0.0)
        for (int k = 1; k <= N; ++k) {
