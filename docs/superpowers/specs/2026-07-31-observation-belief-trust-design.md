...
一筆證據＝三元組 (觀測者＝訊息的 `robot_id`、被觀測者＝`ev_peer[k]`、殘差＝`ev_resid[k]`)，
時間戳沿用訊息既有的 `cycle_id`。長度不符或非有限值的訊息整包丟棄（同現行 Gate 1 語意）。
...
| 行為 | 映射 |
|---|---|
| 協作 | **在 admission 實現，不在迭代內**：b 低於門檻 → 該同儕的宣稱不被 commit（走現行 `block_peer` 路徑），ADMM 更新式一字不改。連續權重進成本函數屬 CBF/QP 數學改動，依專案規則須先與使用者推導，本 spec 不做 |
