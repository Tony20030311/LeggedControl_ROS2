...
- [ ] **Step 3b: Calibrate `refute_ratio` from the same clean-flight run.** Task 7 introduced it by reasoning, not measurement, and it decides whether an honest reporter whose sightline grazes a pillar gets punished. From the clean run, count spurious refutations per agent per minute — reports rejected as geometrically implausible when no attacker exists, caused by grazing sightlines, observation noise and timing skew. Then set the ratio so that **the spurious refutation accumulated over a full mission stays an order of magnitude below the eviction threshold**, and record both the measured rate and the resulting value. If the measured rate is so high that no ratio satisfies that, the margin in the plausibility test is wrong, not the ratio — report rather than shrinking the ratio to hide it.

- [ ] **Step 3c: Show that `obs_range` is not load-bearing, rather than calibrating it.** In simulation the sensing range is a fiction — the stand-in observation is ground truth with no physical limit — so a specific number would be false precision. Instead log the distribution of peer-to-peer distances over a full mission. Formation spacing is 1.40 m and the plum row pitch is about 2.2–2.3 m, so peers should essentially never exceed a few metres; if that holds, every value above the observed maximum gives identical behaviour and the parameter carries no weight in the result. Say so explicitly in the results document — an invariance demonstration is stronger than a tuned number, because it cannot be attacked as an arbitrary choice. **If the distance distribution shows the range DOES bind** (an attacker running away, a survivor detouring wide), then it must be derived from a named sensor model and the paper must state which sensor is being claimed.
